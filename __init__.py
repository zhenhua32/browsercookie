...
class BrowserCookieLoader(object):
    ...
    def load(self, filter_fuc: Callable[[cookielib.Cookie], bool] = None):
        """
        这是啥都要, 加个过滤器
        filter_fuc 应该是一个可调用的函数, 参数是 cookie, 返回 bool.
        如果返回 false 会过滤这个 cookie.
        """
        cookie_jar = cookielib.CookieJar()
        for cookie in self.get_cookies():
            if filter_fuc is not None:
                if not filter_fuc(cookie):
                    continue
    ...
    def _decrypt(self, value, encrypted_value, key):
        ...
        if (sys.platform == "darwin") or sys.platform.startswith("linux"):
            ...
            try:
                ...
                return cipher.decrypt_and_verify(ciphertext, tag).decode("utf-8")  # the decrypted cookie
...
class Safari(BrowserCookieLoader):
    ...
    def find_cookie_files(self):
        if sys.platform != "darwin":  # checks if using OSX
            ...
            )  # no actual use of cookie files because we only have need one specific cookie file
            if cookie_files:
                return cookie_files
            else:
    ...
    def get_cookies(self):
        ...
        for page in pages:
            ...
            for offset in cookie_offsets:
                ...
                expiry_date = str(
                    int(unpack("<d", cookie.read(8))[0] + 978307200)
                )  # 978307200 because mac's time starts at: 2001, 1, 1
